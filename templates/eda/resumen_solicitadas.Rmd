...
for (i in 1:length(OPCIONES_PRES_ONLINE)){
  data <- solic %>% 
          filter(pres_online %like% OPCIONES_PRES_ONLINE[i]) %>%
          mutate(pres_online = OPCIONES_PRES_ONLINE[i])
  data0 <- rbind(data, data0)
}
...
c1 <- c("Mujeres", "Experiencias", "Talleres", "Otros") 
c2 <- c(round(sum(solic$tipo1)/total_solicitudes*100, 1), 
        ...
        round(sum(solic$tipo4)/total_solicitudes*100, 1)
        )
...
<!-- ### Comunidad autónoma y pres_online -->
<!-- ```{r comunidades y pres_online} -->

<!-- tb <- solic %>% -->
<!--         group_by(com_autonoma, pres_online) %>% -->
...
```{r comunidades pres_online e idioma}

tb <- solic %>%
        # group_by(com_autonoma, pres_online, idioma) %>%
...
<!-- ### Provincia y pres_online -->

<!-- ```{r provincia y pres_online} -->

<!-- tb <- solic %>% -->
<!--         group_by(provincia, pres_online) %>% -->
<!--         summarise(nsolic = n_distinct(id), -->
<!--                   ncharlas = n()) %>% -->
<!--         arrange(provincia) -->
...
tb <- solic_idiomas %>%
        # group_by(provincia, pres_online, idioma) %>%
